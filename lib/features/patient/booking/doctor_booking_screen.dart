...
class _DoctorBookingScreenState extends State<DoctorBookingScreen> {
  ...
  void _showBookedDetailsDialog(BookingSlot slot) {
    ...
    showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent,
      builder: (ctx) {
        final rawStr = slot.startAtLocal.isNotEmpty ? slot.startAtLocal : slot.startAt;
        final timeStr = _formatSlotTime(rawStr);
        final dateLabel = DateFormat('EEE, d MMM yyyy').format(_selectedDate);
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.check_circle_rounded, color: Color(0xFF00BFA5), size: 20),
                    ),
                    const SizedBox(width: 12),
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Slot Booked',
                            style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: Color(0xFF00BFA5))),
                    ...
                    const Spacer(),
                    IconButton(
                      icon: Icon(Icons.close_rounded, color: Colors.grey.shade600, size: 25),
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.calendar_today_rounded, size: 14, color: Colors.green.shade700),
                          const SizedBox(width: 8),
                          Text(dateLabel, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: Colors.green.shade900)),
                          const Spacer(),
                          ...
                          const SizedBox(width: 8),
                          Text(timeStr, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: Colors.green.shade900)),
                        ],
                      ),
                    ),
                    const SizedBox(height: 12),
                    ...
                    _CompactInfoTile(
                      icon: Icons.person_rounded,
                      label: 'Patient / Attendee',
                      value: slot.attendeeName ?? 'Unknown',
                      sub: slot.attendeeEmail,
                      color: const Color(0xFF8B5CF6),
                      isDark: isDark,
                      isSubInline: true,
                    ),
                    ...
                    // Type and Status (Side by side compact)
                    Row(
                      children: [
                        Expanded(
                        ...
                        Expanded(
                          child: _CompactInfoTile(
                            ...
                            label: 'Status',
                            value: (slot.bookingStatus ?? 'Confirmed').toUpperCase(),
...
class _SuccessInlineCard extends StatelessWidget {
  final IconData icon;
  final String label;
  final String value;

  const _SuccessInlineCard({
    required this.icon,
    required this.label,
    required this.value,
  });
...
class _SuccessClinicStrip extends StatelessWidget {
  final String clinicName;
  final String clinicLocation;

  const _SuccessClinicStrip({
...
// ── Info Card (Used for confirmation) ──────────────────────────────────────────
